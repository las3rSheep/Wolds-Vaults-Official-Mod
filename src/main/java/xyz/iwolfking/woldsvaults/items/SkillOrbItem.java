...
public class SkillOrbItem extends BasicItem {
    ...
    protected Style getNameColor() {
        return Style.EMPTY.withColor(TextColor.parseColor("#55A757"));
    }

    @Override
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ItemStack heldStack = player.getMainHandItem();
        if(!hand.equals(InteractionHand.MAIN_HAND) || !(heldStack.getItem() instanceof SkillOrbItem)) {
        ...
        if(!level.isClientSide()) {
            ...
            if(server != null) {
                ...
                playerStats.addSkillPoints(1).sync(server);
                heldStack.shrink(1);
