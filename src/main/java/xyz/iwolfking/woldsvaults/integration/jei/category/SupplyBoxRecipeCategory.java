...
public class SupplyBoxRecipeCategory implements IRecipeCategory<SupplyBoxConfig> {
    private static final ResourceLocation TEXTURE = VaultMod.id("textures/gui/jei/loot_info.png");
    public static final RecipeType<SupplyBoxConfig> RECIPE_TYPE = RecipeType.create("woldsvaults", "supply_box_info", SupplyBoxConfig.class);

    private static final TextComponent TITLE = new TextComponent("Supply Box");
    private final IDrawable background;
    private final IDrawable icon;

    public SupplyBoxRecipeCategory(IGuiHelper guiHelper) {
        this.background = guiHelper.createDrawable(TEXTURE, 0, 0, 162, 108);
        this.icon = guiHelper.createDrawableIngredient(VanillaTypes.ITEM_STACK, ModItems.SUPPLY_BOX.getDefaultInstance());
    }
    ...
    @Nonnull
    @Override
    public RecipeType<SupplyBoxConfig> getRecipeType() {
    ...
    @Nonnull @SuppressWarnings("removal")
    public Class<? extends SupplyBoxConfig> getRecipeClass() {
        return this.getRecipeType().getRecipeClass();
    }

    @Override
    @ParametersAreNonnullByDefault
    public void setRecipe(IRecipeLayoutBuilder builder, SupplyBoxConfig recipe, IFocusGroup focuses) {
    ...
    {
        double totalWeight = ModConfigs.SUPPLY_BOX.POOL.getTotalWeight();
        
        CompoundTag nbt = stack.getOrCreateTagElement("display");
        ListTag list = nbt.getList("Lore", 8);

        WeightedList<ProductEntry> entries = ModConfigs.SUPPLY_BOX.POOL;
