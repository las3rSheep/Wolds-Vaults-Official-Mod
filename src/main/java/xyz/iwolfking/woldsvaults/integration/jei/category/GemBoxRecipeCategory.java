...
public class GemBoxRecipeCategory implements IRecipeCategory<GemBoxConfig> {
    private static final ResourceLocation TEXTURE = VaultMod.id("textures/gui/jei/loot_info.png");
    public static final RecipeType<GemBoxConfig> RECIPE_TYPE = RecipeType.create("woldsvaults", "gem_box_info", GemBoxConfig.class);

    private static final TextComponent TITLE = new TextComponent("Gem Box");
    private final IDrawable background;
    private final IDrawable icon;

    public GemBoxRecipeCategory(IGuiHelper guiHelper) {
        this.background = guiHelper.createDrawable(TEXTURE, 0, 0, 162, 108);
        this.icon = guiHelper.createDrawableIngredient(VanillaTypes.ITEM_STACK, ModItems.GEM_BOX.getDefaultInstance());
    }
    ...
    {
        double totalWeight = ModConfigs.GEM_BOX.POOL.getTotalWeight();
        
        CompoundTag nbt = stack.getOrCreateTagElement("display");
        ListTag list = nbt.getList("Lore", 8);

        WeightedList<ProductEntry> entries = ModConfigs.GEM_BOX.POOL;
