        ...
        if (this.getLevel() != null) {
            ...
            if (state.getValue(MonolithBlock.STATE) == MonolithBlock.State.LIT) {
                ...
                if (random.nextInt(15) == 0) {
                    offset = new Vec3(random.nextDouble() / 3.0 * (random.nextBoolean() ? 1 : -1), 0.0, random.nextDouble() / 3.0 * (random.nextBoolean() ? 1 : -1));
                    this.getLevel().addParticle(ParticleTypes.LAVA, true, pos.getX() + 0.5 + offset.x, pos.getY() + random.nextDouble() * 0.15000000596046448 + 0.550000011920929, pos.getZ() + 0.5 + offset.z, offset.x / 12.0, random.nextDouble() * 0.1 + 0.1, offset.z / 12.0);
                }
            }
